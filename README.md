...
### Performance
Evaluation of experiments are conducted on CONLL 2003 English data. The performance has been logged in `mlflow`. There is a concern raised regarding to the quality of CONLL 2003 that we've been using. I obtained the copy of this CONLL dataset on github, whether github author posted the full dataset is unknown and whether the author follows the latest guideline generating the CONLL data is also unknown. Eyeball checking shows that in this dataset many country names are labelled as `I-PER` instead of `I-LOC`, as well as a sentence contains only one single word and the rest 4 or 5 words are random numbers. Further evaluation needed to determine if we can trust results getting from CONLL evaluation. Anyhow, here's the unvalidated performance
